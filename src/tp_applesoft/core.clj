       ...
       (case operador
         ...
         * (if (and (number? operando1) (number? operando2))
         ...
         - (if (and (number? operando1) (number? operando2))
             (- operando1 operando2)
             (dar-error 163 nro-linea))
         (symbol "^") (if (and (number? operando1) (number? operando2))
                        (Math/pow operando1 operando2)
                        (dar-error 163 nro-linea))
...
(defn palabra-reservada? [x]
  (contains? #{"LOAD",
               ...
               "STR$",
               "AND",
               "OR"} (clojure.string/upper-case x)))
...
(defn anular-invalidos [sentencia]
  (map
   (fn [simbolo] (cond
                   (nil? simbolo) nil
                   (contains? simbolos-invalidos (str simbolo)) nil
                   :else simbolo))
   sentencia))
...
(defn cargar-linea [linea amb]
  ; Se inserta la linea en la lista de la posicion 0 del ambiente,
  ; ordenando por numero de linea.
  ; Si una linea con ese numero ya existe, se elimina y luego se inserta la nueva.
  (assoc amb
         0
         (sort-by first
                  <
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defn expandir-next [n]
  ; (A , B) => ((NEXT A) (NEXT B))
  (if (empty? n)
    (list 'NEXT)
    (map (fn [x] (list 'NEXT x)) (remove (fn [x] (= x (symbol ","))) n))))

(defn flatten-first-level [l]
  (apply concat (map (fn [x] (if (list? (first x)) x (list x))) l)))

(defn expandir-nexts [n]
  (flatten-first-level (map (fn [sentencia]
                              (if (= (first sentencia) 'NEXT)
                                (expandir-next (rest sentencia))
                                sentencia))
                            n)))
...
(defn dar-error [cod prog-ptrs]
  ...
  (if (not= :ejecucion-inmediata (first prog-ptrs))
    (print (apply str [" IN ", (first prog-ptrs)])))
  nil)
...
(defn variable-integer? [x]
  (and (= '\% (last (name x))) (nombre-variable-valido? x)))
...
(defn contar-sentencias [nro-linea amb]
  (let [linea (remove #(= nro-linea %)
                      (first
                       (filter #(= nro-linea (first %))
                               (first amb))))]
    (count (expandir-nexts linea))))
...
(defn buscar-lineas-restantes
  ...
  ([ptr sentencias]
   (let [nro-linea (first ptr) ,
         sentencias-restantes (second ptr)]
     (if
      (or
       (= 0 (count (filter #(= nro-linea (first %)) sentencias))) ; si no existe la liena
       (= :ejecucion-inmediata nro-linea)) ; si es ejecucion inmediata
       nil
       (let [linea-actual (first (filter #(= nro-linea (first %)) sentencias)) ,
             lineas-siguientes (filter #(< nro-linea (first %)) sentencias)]
         (concat
          (list
           (cons nro-linea
                 (take-last sentencias-restantes (expandir-nexts (drop-while number? linea-actual))))) ; las restantes sentencias de la linea actual
          lineas-siguientes)))))) ; lineas siguientes
...
(defn continuar-linea [amb]
  (if
   (empty? (nth amb 2)) ; si gosub-return-stack vacio, error
    (do (dar-error 22 (nth amb 1)) (vector nil amb))
    (let [gosub-return-stack (nth amb 2)
          nuevo-prog-ptr [(first (peek gosub-return-stack)) (- (second (peek gosub-return-stack)) 1)]
          nuevo-gosub-return-stack (pop gosub-return-stack)]
      (vector :omitir-restante [(nth amb 0) nuevo-prog-ptr nuevo-gosub-return-stack (nth amb 3) (nth amb 4) (nth amb 5) (nth amb 6)]))))
...
(defn no-empieza-con-rem? [x]
  (not (or (nil? x) (= 'REM (first x)))))

(defn remover-sentencias-rem [linea]
  ; Remueve las sentencias rem y sus posteriores de una linea. Ej:
  ; '(10 (PRINT X) (REM ESTE NO) (DATA 30)) => (10 (PRINT X))
  (cons (first linea) (take-while no-empieza-con-rem? (drop-while number? linea))))

(defn into-str [x]
  ; si x es un simbolo, lo castea a str. Si es un numero, no.
  (if (number? x) x (str x)))

(defn procesar-data [linea]
  ; Para todas las sentencias de una linea, devuelve el contenido de DATA. Ej:
  ; (list 100 (list 'DATA 'MUNDO (symbol ",") 10 (symbol ",") 20))) => ("MUNDO" , 10 , 20)
  (let [sentencias (rest linea)]
    (mapcat (fn [l] (if (= 'DATA (first l)) (map into-str (rest l)) nil)) sentencias)))

(defn extraer-data [prg]
  (->> prg
       (map remover-sentencias-rem)
       (mapcat procesar-data)
       (remove nil?)
       (remove #(= "," (str %)))))
...
(defn ejecutar-asignacion [sentencia amb]
  (let
   [variable (first sentencia)
    vars-ambiente (amb 6)
    cant-variables-sentencia (count (filter nombre-variable-valido? sentencia))]
    (if (>= cant-variables-sentencia 1)
      (assoc amb 6 (assoc vars-ambiente variable (calcular-expresion (drop 2 sentencia) amb)))
      amb)))
...
; (5 + 0 / 2 * 0)
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defn default [variable]
  (cond
    (string? variable) variable ; tiene que chequearse primero si es string
    (variable-float? variable) 0
    (variable-integer? variable) 0
    (variable-string? variable) ""
    :else variable))

(defn get-variable [vars-ambiente variable]
  (let [valor (get vars-ambiente variable nil)]
    (if (= nil valor)
      (default variable)
      valor)))

(defn procesar-elemento-expresion [amb x]
  (let [resultado (if (nombre-variable-valido? x)
                    (get-variable (amb 6) x)
                    x)]
    (if (= '. resultado)
      0
      resultado)))
...
(defn preprocesar-expresion [expr amb]
  (map (partial procesar-elemento-expresion amb) expr))
...
(defn es-mid-ternario [n expresion]
  (= 2 (count (filter #(= "," (str %))
                      (take-while #(not= ")" (str %)) (subvec expresion n))))))

(defn no-es-unario [anterior]
  (and
   (not= :sin-anterior anterior)
   (or (= (symbol ")") anterior) (number? anterior) (string? anterior))))

(defn desambiguar-elemento [n expresion]
  (let
   [prev (get expresion (- n 1) :sin-anterior)
    actual (get expresion n :fin)]
    (cond
      (= :fin actual) expresion ; condicion de corte
      (= (symbol "+") actual) (if (no-es-unario prev)
                                (desambiguar-elemento (+ n 1) expresion)
                                (desambiguar-elemento (+ n 1) (assoc expresion n nil)))
      (= (symbol "-") actual) (if (no-es-unario prev)
                                (desambiguar-elemento (+ n 1) expresion)
                                (desambiguar-elemento (+ n 1) (assoc expresion n (symbol "-u"))))
      (= (symbol "MID$") actual) (if (es-mid-ternario (+ n 2) expresion)
                                   (desambiguar-elemento (+ n 1) (assoc expresion n (symbol "MID3$")))
                                   (desambiguar-elemento (+ n 1) expresion))
      :else (desambiguar-elemento (+ n 1) expresion))))

(defn desambiguar [expr]
  (filter
   #(not (nil? %)) ; los + unarios se reeplazan por nil y se filtran
...
(defn eliminar-ceros-finales [n]
  (Double/parseDouble (apply str (reverse (drop-while #(= \0 %) (reverse (str n)))))))

(defn eliminar-cero-decimal [n]
  (if (number? n)
    (if (es-entero? n)
      (int n)
      (eliminar-ceros-finales n))
    n))
...
; "-.5"
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defn eliminar-ceros-iniciales [x]
  (reduce str (if (neg? x)
                (cons \- (drop-while #(= \0 %) (drop 1 (str x))))
                (drop-while #(= \0 %) (str x)))))
...
(defn eliminar-cero-entero [n]
  (cond
    ...
    (symbol? n) (str n)
    (es-entero? n) (str n)
    :else (eliminar-ceros-iniciales n)))
